...
def fill_excel(mes1, mes2, mes3):
    ...
    worksheet.set_column(0, 0, 27.29)
    worksheet.set_column(1, 1, 19.71)
    worksheet.set_column(2, 2, 36.14)
    worksheet.set_column(3, 3, 14.14)
    worksheet.set_row(0, 42)
    
    header_format = workbook.add_format({'font_name': 'Times New Roman', 'font_size': 12, 'text_wrap': True, 'align': 'center', 'valign': 'center'})
    ...
    worksheet.write('A3', "Прізвище ім'я по батькові", table_cell)
    ...
    worksheet.write(row + 5, 2,  mes1, text_format)
    worksheet.write(row + 6, 2,  mes2, text_format)
    worksheet.write(row + 7, 2,  mes3, text_format)
